...
pub fn next_state(current_state :&ParsingState, tt :&TokenTree) -> ParsingState
{
    match current_state {
        ...
        ParsingState::Type => match tt {
            TokenTree::Group(group) => if group.span_open().source_text().expect("init group has no starting param") == "("
            {return ParsingState::InitInfo} 
            else {punct_panic(tt, current_state)},
        ...
        ParsingState::VarName => match tt {
            TokenTree::Group(group) => if group.span_open().source_text().expect("Val has no starting param") == "("
            {return ParsingState::Value} 
            else {punct_panic(tt, current_state)},
